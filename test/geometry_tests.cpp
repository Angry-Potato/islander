...
TEST_SUITE("Geometry::squaredDistanceBetween") {
  TEST_CASE("Vectors both on the X axis") {
    ...
    SUBCASE("Distance between should just be difference in X") {
      CHECK(Geometry::squaredDistanceBetween(v1, v2) == v2X - v1X);
    }
  }
  TEST_CASE("Vectors both on the Y axis") {
    ...
    SUBCASE("Distance between should just be difference in Y") {
      CHECK(Geometry::squaredDistanceBetween(v1, v2) == v2Y - v1Y);
    }
  }
  TEST_CASE("Vectors are at the same position") {
    ...
    SUBCASE("Distance between should be 0") {
      CHECK(Geometry::squaredDistanceBetween(v1, v2) == 0);
    }
  }
  TEST_CASE("Vectors at different positions") {
    ...
    SUBCASE("Distance between should be difference in X + difference in Y") {
      ...
      CHECK(Geometry::squaredDistanceBetween(v1, v2) == expected);
    }
    SUBCASE("Distance between should be the same, regardless of order") {
      CHECK(Geometry::squaredDistanceBetween(v1, v2) == Geometry::squaredDistanceBetween(v2, v1));
    }
  }
}
